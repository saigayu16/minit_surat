...
// 2. AMBIL NAMA USER (Gunakan fallback jika session tidak diset)
$user_name = $_SESSION['user_name'] ?? 'Admin Sistem';

// 3. KIRA STATISTIK (Kriteria Admin Berdasarkan Status 'SELESAI')
$total_surat = 0;
$total_wait = 0;
$total_done = 0;
...
<head>
    ...
    <style>
        ...
        /* Statistik / Kriteria Utama Admin */
        .admin-stats {
            display: grid;
        ...
        /* Jadual Dokumen (Gaya Minimalis Moden) */
        .table-container {
            background: var(--card-bg);
            border-radius: 12px;
            box-shadow: 0 4px 6px -1px rgba(0,0,0,0.02), 0 2px 4px -1px rgba(0,0,0,0.02);
        ...
        .dimaklum::before { content: '●'; color: #4338ca; }
    </style>
</head>
<body>
...
<nav class="navbar">
    ...
    <div class="user-info">
        <span class="avatar-icon">
            <i class="fa-solid fa-user-shield"></i> &nbsp;<?= htmlspecialchars($user_name) ?>
        </span>
        | 
...
<div class="container">
    ...
    <div class="section-header">
        ...
        <a href="daftar_surat.php" class="btn-add">
            <i class="fa-solid fa-plus"></i> Daftar Surat Baru
        </a>
    </div>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th>Perkara</th>
                    <th>Didaftar Oleh</th>
                    <th>Status Tandatangan</th>
                    <th>Tindakan</th>
                    <th>Maklum Kepada Staf</th>
                </tr>
            </thead>
           <tbody>
                <?php
                ...
                if ($res && $res->num_rows > 0) {
                    while($row = $res->fetch_assoc()) {
                        ...
                        $maklum_staf = htmlspecialchars($row['maklum_kepada'] ?? '-');
                        
                        // Logik Badge Status
                        if (strcasecmp($status, 'SELESAI') == 0) {
                            $badge = 'done';
                            $display_status = 'SELESAI';
                        } elseif (strcasecmp($status, 'DIMAKLUM') == 0) {
                            $badge = 'dimaklum'; // Pastikan CSS .dimaklum ada dalam style anda
                            $display_status = 'DIMAKLUM';
                        } else {
                            $badge = 'wait';
                            $display_status = $status;
                        }
                        
                        $admin_name = htmlspecialchars($row['didaftarkan_oleh'] ?? 'Tiada Data');
                        
                        // Menentukan pautan tindakan
                        if (strcasecmp($status, 'SELESAI') == 0) {
                            $tindakan = "<a href='cetak_minit.php?id={$row['id']}' target='_blank' class='action-link' style='color:#16a34a;'><i class='fa-solid fa-print'></i> Cetak</a>";
                        } else {
                ...
                // Gantikan bahagian echo yang lama dengan kod ini:

echo "<tr>
    <td style='font-weight:500;'>".date('d/m/Y', strtotime($row['tarikh_terima']))."</td>
    <td>".htmlspecialchars($row['no_rujukan'] ?? '-')."</td>
    <td>".htmlspecialchars($row['daripada'] ?? '-')."</td>
    <td>".htmlspecialchars($row['perkara'] ?? '-')."</td>
    <td><i class='fa-solid fa-user-pen'></i> {$admin_name}</td>
    <td><span class='status-badge {$badge}'>{$display_status}</span></td>
    <td>{$tindakan}</td>
    <td style='text-align: center; vertical-align: middle;'>
        <div style='display: flex; flex-direction: column; align-items: center; gap: 6px;'>
            <!-- Tag Bukti Makluman (Warna Merak/Teal) -->
            <span style='background: #e6fffa; color: #0d9488; padding: 3px 10px; border-radius: 6px; font-size: 0.75rem; font-weight: 700; border: 1px solid #99f6e4; box-shadow: 0 1px 2px rgba(0,0,0,0.05);'>
                " . htmlspecialchars($row['maklum_kepada'] ?? '-') . "
            </span>
            <!-- Butang Maklum -->
            {$maklum_link}
        </div>
                    ...
                    echo "<tr><td colspan='9' style='text-align:center; padding: 30px; color: var(--text-muted);'>📂 Tiada senarai dokumen rasmi didaftarkan lagi buat masa ini.</td></tr>";
                }
                ?>
            </tbody>
        </table>
