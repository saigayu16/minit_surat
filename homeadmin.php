...
<head>
    ...
    <style>
        ...
        .selesai-tanda { background: #e0e7ff; color: #4338ca; }
        .action-link { text-decoration: none; font-size: 0.85rem; font-weight: 600; display: block; margin-bottom: 5px; }
    </style>
</head>
<body>
            ...
            <tbody>
                ...
                while($row = $res->fetch_assoc()) {
                    ...
                    echo "<tr>
                        ...
                        <td>
                            <a href='view_surat.php?id={$row['id']}' class='action-link' style='color:#2563eb;'><i class='fa-solid fa-eye'></i> Lihat</a>
                            ".($status == 'SELESAI TANDATANGAN' ? "<a href='cetak_minit.php?id={$row['id']}' target='_blank' class='action-link' style='color:#16a34a;'><i class='fa-solid fa-print'></i> Cetak</a>" : "")."
                        </td>
                        <td>" . (!empty($row['maklum_kepada']) ? "<span style='color:#0369a1; font-weight:bold;'>".$row['maklum_kepada']."</span>" : "<a href='maklum.php?id={$row['id']}' style='color:#7c3aed;'>Maklum</a>") . "</td>
                    </tr>";
                }
                ?>
            </tbody>
