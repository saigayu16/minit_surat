...
<head>
    ...
    <style>
        ...
        body { font-family: 'Inter', sans-serif; background-image: linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url('homeadmin.jpg'); background-size: cover; background-attachment: fixed; margin: 0; padding: 0; color: var(--text-main); }
        ...
        th { background: #f8fafc; color: var(--text-muted); padding: 16px; font-size: 0.85rem; text-transform: uppercase; border-bottom: 1px solid var(--border-color); }
        ...
        .btn-view { padding: 6px 12px; background: #e0e7ff; color: #4338ca; border-radius: 4px; text-decoration: none; font-size: 0.8rem; font-weight: 600; }
        .btn-daftar { background: #059669; color: white; padding: 10px 18px; border-radius: 6px; text-decoration: none; font-size: 0.9rem; font-weight: 600; }
    </style>
</head>
<body>
...
<nav class="navbar">
    ...
    <div class="header-actions">
        <span>Admin: <strong><?= htmlspecialchars($user_name) ?></strong></span>
        <a href="daftar_surat.php" class="btn-daftar"><i class="fa-solid fa-plus"></i> Daftar Surat</a>
        <a href="logout.php" style="color:#f87171; text-decoration:none; margin-left:20px;"><i class="fa-solid fa-right-from-bracket"></i> Log Keluar</a>
    </div>
</nav>
...
<div class="container">
    ...
    <div class="table-container">
        <table>
            <thead>
                <tr><th>Tarikh</th><th>No. Rujukan</th><th>Daripada</th><th>Status</th><th>Tindakan</th><th>Maklum Kepada</th></tr>
            </thead>
            <tbody>
                <?php
                ...
                while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    echo "<tr>
                        <td>".($row['tarikh_terima'] ? date('d/m/Y', strtotime($row['tarikh_terima'])) : '-')."</td>
                        <td>".htmlspecialchars($row['no_rujukan'] ?? '-')."</td>
                        <td>".htmlspecialchars($row['daripada'] ?? '-')."</td>
                        <td><span class='status-badge {$badge}'>{$status}</span></td>
                        <td><a href='view_surat.php?id={$row['id']}' class='btn-view'>Lihat</a></td>
