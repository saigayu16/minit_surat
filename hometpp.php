...
// 2. KIRA STATISTIK KHAS TPP (Guna PDO)
$total_perlu_sahkan = 0;
$total_selesai = 0;
$total_semua = 0;

// Kira Dokumen Menunggu
$stmt1 = $conn->prepare("SELECT COUNT(*) as total FROM minit_surat WHERE target_role = 'tpp' AND status NOT LIKE ?");
$stmt1->execute(['%Sudah Disahkan%']);
$total_perlu_sahkan = $stmt1->fetch(PDO::FETCH_ASSOC)['total'];

// Kira Dokumen Selesai
$stmt2 = $conn->prepare("SELECT COUNT(*) as total FROM minit_surat WHERE target_role = 'tpp' AND status LIKE ?");
$stmt2->execute(['%Sudah Disahkan%']);
$total_selesai = $stmt2->fetch(PDO::FETCH_ASSOC)['total'];

// Kira Jumlah Keseluruhan
$stmt3 = $conn->prepare("SELECT COUNT(*) as total FROM minit_surat WHERE target_role = 'tpp'");
$stmt3->execute();
...
<head>
    ...
    <style>
        /* CSS dikekalkan sama seperti asal */
...
<div class="container">
    ...
    <div class="table-container">
        <table>
            ...
            <tbody>
                <?php
                $stmt = $conn->prepare("SELECT * FROM minit_surat WHERE target_role = 'tpp' ORDER BY id DESC");
                $stmt->execute();
                ...
                if ($rows) {
                    foreach ($rows as $row) {
                        ...
                        echo "<tr>
                            ...
                            <td>{$row['kolej']}</td>
                            <td>{$row['perkara_surat'] ?? $row['perkara']}</td>
                    ...
                    echo "<tr><td colspan='6' style='text-align:center;'>Tiada dokumen untuk TPP.</td></tr>";
                }
                ?>
            </tbody>
        </table>
